...
- **Game shell** (spec §11 M6) — **DONE**: a terminal game (`game/`) on top of the engine — discover materials by combining (deterministic, with coined alloy names), inspect properties, build machines through a generic catalog over `machines/`, and progress through prerequisite-gated goals that auto-complete from real measured properties. Tiny deterministic saves (replay the combo list, no lattices). `python -m game`; `tests/test_game.py`. `game/` imports engine+machines, never the reverse. See README "Game shell findings". Natural follow-ups: more goals/machines, a chemistry-synthesis track in-game (the reachability tech-tree), or a web UI on the same UI-agnostic `Session` core.
...
- **M7a — Spectral / band gap: DONE** — revived on the chemistry layer's dense (fill=1.0) rock-salt crystals (the substrate the first, falsified attempt lacked). Tight-binding ionic gap `= 2Δ` with `Δ ∝ ΔEN`, parameter-free and `t`-independent; new per-cell `site_potential` field (out of signature → M0–M8 byte-identical), `engine/properties/spectral.py`, gated `band_gap`, `chem_explorer spectral`, `tests/test_spectral.py`. The gap is honestly brittle to vacancies (a crossover, not a transition). **M7b (covalent Si/diamond semiconductors) deferred** — needs the 3D substrate / SSH bond-alternation; a genuine fork awaiting the user. See README "M7a findings" and `memory/m7a-spectral-status.md`.
